...
const VehicleDetail = () => {
 ...
 const { data: vehicle, isLoading, error, refetch } = useVehicles(id);
 ...
 const { mutate: updateVehicle } = useUpdateVehicle();
  ...
  const handleSubmit = (data: Partial<typeof vehicle>) => {
    if (!vehicle) return;

    const updateVehicle = { ...vehicle, ...data };
    updateVehicle(
      { id: vehicle.id!, vehicle: updateVehicle },
      {
        onSuccess: () => {
          refetch();
          setEditing(false);
