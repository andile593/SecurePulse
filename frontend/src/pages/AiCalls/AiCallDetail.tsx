...
const AiCallDetail = () => {
  ...
  const handleSubmit = (data: Partial<AiCall>) => {
    updateAiCall({ id: aiCall.id!, aiCall: { ...aiCall, ...data } }, {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      {editing ? (
        <AiCallForm initialData={aiCall} onSubmit={handleSubmit} onClose={() => setEditing(false)} />
      ) : (
        <>
          ...
          <div className="flex gap-4 mt-6">
            <button className="bg-primary-600 text-white px-4 py-2 rounded hover:bg-blue-700" onClick={() => setEditing(true)}>Edit</button>
            <button className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700" onClick={handleDelete}>Delete</button>
            <button className="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400" onClick={() => navigate('/ai-calls')}>Back</button>
          </div>
        </>
      )}
    </div>
